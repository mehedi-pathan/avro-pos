...
const pageVariants = {
  ...
  exit: { opacity: 0, x: -24, transition: { duration: 0.15, ease: "easeIn" as const } }
};
...
export function MainLayout({ title, children }: { title: string; children: ReactNode }) {
  ...
  const content = (
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: "var(--bg-app)" }}>
      <aside className="flex h-full w-[260px] shrink-0 flex-col border-r border-[var(--border-default)] bg-[var(--bg-sidebar)] shadow-2xl backdrop-blur-xl">
          ...
          <div className="p-5">
            ...
            <nav className="space-y-1">
              {navItems
                ...
                .map((item) => (
                  <Link
                    className={`flex items-center gap-3 rounded border px-3 py-2.5 text-sm transition-all ${
                      isActive(pathname, item.href)
                        ? "border-teal/40 bg-teal/10 text-teal"
                        : "border-transparent text-[var(--text-default)] hover:border-[var(--border-default)] hover:bg-[var(--bg-card)] hover:text-[var(--text-primary)]"
                    }`}
                    href={item.href}
                    key={item.href}
                  >
        ...
        <div className="border-t border-[var(--border-default)] p-5">
          ...
          {/* System Status */}
          <div className="mt-4 rounded border border-[var(--border-default)] bg-[var(--bg-overlay)] p-3 text-xs">
            <div className="flex items-center gap-2 mb-0.5">
              <span className={`inline-block h-2 w-2 rounded-full ${heartbeat?.sqlite === "online" ? "bg-emerald-400" : "bg-red-400"} shadow-sm`} style={{ boxShadow: heartbeat?.sqlite === "online" ? "0 0 6px rgba(52,211,153,0.6)" : "0 0 6px rgba(248,113,113,0.6)" }} />
              <span className="font-medium text-[var(--text-primary)]">All Systems Operational</span>
            </div>
            <p className="text-[10px] text-[var(--text-tertiary)] ml-4">Everything is running smoothly</p>
          </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-[var(--border-default)] bg-[var(--bg-card)] px-5 backdrop-blur-xl" style={{ background: "var(--bg-card)" }}>
